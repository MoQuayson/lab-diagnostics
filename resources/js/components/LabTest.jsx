...
class LabTest extends Component{
    ...
    SetAppointmentDetails(id,date){
        document.getElementById('update_test_id').value = id;
                ...
                return (
                    items.map((item, index) => (
                        <tr key={index}>
                            ...
                            <td>{item.fullname}</td>
                            <td>{item.name}</td>
                            <td><a href={'/admin/lab-test/'+item.id}>{item.id}.pdf<i className='bi bi-box-arrow-up-right ms-1'></i></a></td>
                            <td>{item.price}</td>
    ...
    //fetch details
    DataFetch(){
        var url = window.location.origin + '/get-tests';
        fetch(url)
        .then(res => res.json())
        .then(
            (result) => {
    ...
    handlePageChange(pageNumber) {
        ...
        axios.get('/get-tests?page='+pageNumber).then((response)=>{
          this.setState({
    ...
    render(){
        ...
        return(
            <div className='card mb-5 shadow'>
                ...
                <UpdateLabTestModal isAdmin={false}/>
                <DeleteLabTestModal isAdmin={false}/>
...
if (document.getElementById('test_list')) {
    ReactDOM.render(<LabTest />, document.getElementById('test_list'));
}
